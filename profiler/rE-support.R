...
assembleSector <- function(load.segments, t.weights=rep(1,length(load.segments)), name='Sector') {
  ...
  revenueByOwner <- function(p.vect, pc, owner) {
    if (!owner %in% owner.names) return(0)
    sub.revenue <- sapply(1:n, function(i) { load.segments[[i]]$owners[[owner]]$revenue(p.vect[i], pc) } )
    sum(t.weights*sub.revenue)
  }
...
assembleIndustry <- function(sectors, name='Industry', pc.precalc=seq(0, 150, 0.5)) {
  ...
  revenueByOwners <- function(p.list, pc, owners) { 
    owner.revenue <- lapply(owners, function(owner) sum(sapply(1:n, function(i) 
      sectors[[i]]$revenueByOwner(p.list[[i]], pc, owner))))
    names(owner.revenue) <- owners
  ...
  return(list(sectors=sectors, sector.weights=sector.weights,
              ...
              profitByOwners=profitByOwners, revenueByOwners = revenueByOwners,
              name=name,
              demand=demand,
              sector.count=n,
...
makeNercIndustry <- function(pd, load.data, load.groups = c(0,1), elasticity=-1, 
                             pc.precalc=seq(0, 150, 0.5), 
                             ramp=0.5, owner.column='ParentPAC') {
  ...
  assembleIndustry(sectors, pc.precalc=pc.precalc)
}
